...
- **M3** Android 端（未开始）：Kotlin + CameraX + ML Kit，协议对齐
- **M4** 可靠性增强（已完成）：缺块可视化、人工补发、两端断点续传
- **M5** 加速项（部分）：2×2 多码网格已做；彩色 QR、参数面板待续

## 下一步建议

M4 可靠性增强（缺块可视化、人工补发、断点续传）已完成，全部通过合成 QR 闭环与 offscreen GUI 单测验证。下一步风险仍在真实光学链路。

优先级：

1. 建立真实 PC -> PC 测试矩阵：文件大小、FPS、ECC、网格、距离、屏幕亮度、摄像头分辨率。
2. 记录吞吐量、完成率、平均恢复帧数和失败样本，验证人工补发与断点续传在真实丢帧下的有效性。
3. 补齐 GUI 吞吐量/误码率统计与真实链路调参向导（M5 参数面板）。
4. 稳定 PC 链路后，再推进 Android 端协议对齐（M3）。
