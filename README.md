...
通过「屏幕显示二维码 + 摄像头采集」的光学信道，在物理隔离网络之间做单向数据摆渡。当前实现已经打通协议核心、标准 QR 编解码、PC 发送/接收 GUI、文本/文件接收和自动化测试。
...
- 内容类型：支持文本与文件传输；文件校验完成后由用户自定义路径和名称保存。
- 完整性校验：接收完成后按 SHA-256 校验原始数据。
- QR 后端：使用 `segno` 编码、`zxing-cpp` 解码；发送端自适应压缩分片后以系统化 LT 喷泉流持续发送（先直发 K 个源块、再无限生成随机组合符号，漏帧无需等轮询补缺），周期重发 MANIFEST，作为默认文件传输链路。
- 彩色码后端：QFC1 四色矩阵使用颜色标定和分块 Reed-Solomon 纠错，保留为受控环境实验模式；默认文件传输使用标准 QR。
- PC GUI：PySide6 发送/接收双页、OpenCV 摄像头采集、进度显示、文本复制、文件另存。
...
- 设置摄像头索引；程序以 30 FPS 为上限自动采集。
- 点击开始接收后，将摄像头对准发送端 QR。
- 文本完成后显示在结果框；文件完成后点击“保存文件”，可自定义路径和名称。
