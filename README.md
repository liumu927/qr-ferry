...
**PC 端（Python / PySide6）**
- 发送 / 接收双页 GUI，OpenCV 摄像头采集，FPS / 纠错级别 / 帧长 / 冗余 / 网格可调。
...
- **发送**：文本与文件均可发送；标准 QR（推荐）/ 彩色码（实验）切换；FPS 5~30 多档可调；帧长 700 / 1200 / 2200 B（源块档位随帧长联动 512 / 1024 / 2048 B）；纠错 L / M；帧生成与渲染在后台线程，主线程只贴图；发送时自动拉满屏幕亮度并保持唤醒。
...
- **发送页**：选择文件或输入文本，设置 FPS / 纠错级别（默认 L）/ 帧长（700 / 1200 / 2200 B，源块档位随帧长联动）/ 冗余 / 网格，开始发送后窗口持续渲染 QR 帧流。
- **接收页**：摄像头列表自动探测可用设备（可手动刷新），开始接收后将摄像头对准发送端 QR；解码在后台线程进行，采集不阻塞（设备支持时按 60 FPS 采集以降低时间混帧）；文本完成显示在结果框，文件完成后点「保存文件」自选路径。
...
- 模式切换、编码（标准 QR / 彩色码）、FPS（5~30 多档）、帧长（700 / 1200 / 2200 B）、纠错（L / M）均在界面内选择。
...
- `tools/qr_loopback_bench.py`：标准 QR 参数寻优基准，仿真"屏幕 → 相机"链路（时间混帧 / 模糊 / 噪声），比较帧长 × 纠错级 × 帧率 × 相机曝光组合的有效吞吐估计。
