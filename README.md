...
## 里程碑
- **M1** 协议核心层（进行中）：CRC32 / 帧编解码 / 分块 / LT 喷泉码 / 会话 / 端到端自测
- **M2** PC 双端打通：OpenCV 采集 + ZXing 解码 + qrcode 渲染，PC↔PC 链路
- **M3** Android 端（Kotlin + CameraX + ML Kit），协议对齐
- **M4** 进度与漏帧补传、SHA-256 校验、文件原名落盘、文本复制
- **M5** 加速项：多码网格、彩色 QR、参数面板

## 下一步建议

建议先进入测试阶段，而不是继续堆功能。当前自动化测试已覆盖协议和合成 QR 闭环，但项目风险主要在真实光学链路。

优先级：

1. 建立真实 PC -> PC 测试矩阵：文件大小、FPS、ECC、网格、距离、屏幕亮度、摄像头分辨率。
2. 记录吞吐量、完成率、平均恢复帧数和失败样本，确定默认参数是否可靠。
3. 基于测试结果再开发漏帧/缺块可视化、人工补传和参数面板。
4. 稳定 PC 链路后，再推进 Android 端协议对齐。
